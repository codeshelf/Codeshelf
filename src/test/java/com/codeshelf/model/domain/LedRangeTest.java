...
public class LedRangeTest extends MinimalTest {
	...
	public final void rangeTest() {
		...
		ledRange = LedRange.computeLedsToLight(1, 80, true, 2.5, 0.5, false);
		assertRange(ledRange, 15, 18);

		// invert the led direction
		ledRange = LedRange.computeLedsToLight(1, 80, false, 2.5, 0.5, false);
		assertRange(ledRange, 63, 66);

		// common case of cmFromLeft not being set yet, so value is 0.0
		ledRange = LedRange.computeLedsToLight(1, 80, true, 2.5, 0.0, false);
		assertRange(ledRange, 39, 42);

		// Same, but the Double is null rather than coming as 0.0
		ledRange = LedRange.computeLedsToLight(1, 80, true, 2.5, null, false);
		assertRange(ledRange, 39, 42);

		// Edge cases
		// lighting 4, but range is only 3
		ledRange = LedRange.computeLedsToLight(1, 3, true, 2.5, 0.5, false);
		assertRange(ledRange, 1, 3);

		ledRange = LedRange.computeLedsToLight(1, 3, false, 2.5, 0.5, false);
		...
		ledRange = LedRange.computeLedsToLight(1, 80, true, 2.5, 0.01, false);
		assertRange(ledRange, 1, 2);

		ledRange = LedRange.computeLedsToLight(1, 80, false, 2.5, 0.01, false);
		assertRange(ledRange, 79, 80);

		// lighting 4, but on high edge
		ledRange = LedRange.computeLedsToLight(1, 80, true, 2.5, 2.49, false);
		assertRange(ledRange, 79, 80);
		ledRange = LedRange.computeLedsToLight(1, 80, false, 2.5, 0.01, false);
		assertRange(ledRange, 79, 80);

		// various out of range. These error cases return zero values. Only some are logged
		ledRange = LedRange.computeLedsToLight(0, 0, true, 2.5, 0.5, false);
		assertRange(ledRange, 0, 0);

		// this next line will log an error for meters from anchor greater than width.
		ledRange = LedRange.computeLedsToLight(1, 80, false, 2.5, 3.0, false);
		assertRange(ledRange, 0, 0);

		ledRange = LedRange.computeLedsToLight(-10, 80, false, 2.5, 0.5, false);
		assertRange(ledRange, 0, 0);

		ledRange = LedRange.computeLedsToLight(80, 1, false, 2.5, 0.5, false);
		assertRange(ledRange, 0, 0);
		...
		ledRange = LedRange.computeLedsToLight(36, 39, true, 2.5, 0.0, false);
		assertRange(ledRange, 36, 39);

		// Non-slotted case??? but too many leds. Light the middle 4
		ledRange = LedRange.computeLedsToLight(29, 39, true, 2.5, 0.0, false);
