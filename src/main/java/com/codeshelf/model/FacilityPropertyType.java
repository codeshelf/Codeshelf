...
public enum FacilityPropertyType {
	...
	AUTOSHRT("true","On CHE setup, if there is no product known by the system, shall the system immediately short and not present those work instructions. Some sites may like this off, making workers locate and update the item location immediately.", "true, false."),
	LIGHTCLR("RED","When a user lights a location or item, what color shall it light", "Red, Green, Blue, Cyan, Orange, Magenta, White."),
	LOCAPICK("false","Shall inventory and pick locations come from preferredLocation field on order detail.", "true, false."),
	EACHMULT("false","Shall multiple inventory locations for EACH SKUs be allowed in the facility. (Will decrement or change eventually in favor of work area controls.)", "true, false."),
	PICKINFO("SKU","The secondary information shown on the CHE during a pick, below the location.", "SKU, Description, Both."),
	CNTRTYPE("Order","The content of the CHE containers. This affects the message displayed during CHE setup.", "Order, Container."),
	...
	ORDERSUB("Disabled","Specification of the sub-string to capture from scanned order/container. A value of 11-19 means to take characters 11 through 19 as the sub-string. This parameter takes effect after the site controller restarts.", "Disabled, <start>-<end>."),
