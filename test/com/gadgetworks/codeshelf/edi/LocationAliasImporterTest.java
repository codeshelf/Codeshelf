...
public class LocationAliasImporterTest extends EdiTestABC {
	...
	public final void successEventsProduced() {
		...
		verify(producer, times(2)).produceEvent(eq(EnumSet.of(EventTag.IMPORT, EventTag.LOCATION_ALIAS)),
			eq(EventSeverity.INFO),
			any(ImportCsvBeanABC.class));
		verify(producer, Mockito.never()).produceViolationEvent(any(Set.class),
			any(EventSeverity.class),
			any(Exception.class),
			any(Object.class));
	...
	public final void violationEventProducedWhenLocationInactive() {
		...
		ICsvLocationAliasImporter importer = new LocationAliasCsvImporter(producer, mLocationAliasDao);
		importer.importLocationAliasesFromCsvStream(new StringReader(csvString), facility, ediProcessTime);
		verify(producer, times(1)).produceViolationEvent(eq(EnumSet.of(EventTag.IMPORT, EventTag.LOCATION_ALIAS)),
			eq(EventSeverity.WARN),
			any(Errors.class),
			any(ImportCsvBeanABC.class));
